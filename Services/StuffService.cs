...
public class StuffService : IStuffService
{
    ...
    private ILogger<StuffService> _logger;
    public StuffService(ShopDbContext context,
        ILogger<StuffService> logger)
    {
        _logger = logger;
        _context = context;
    }
    public async Task<GetStuffResponse> CreateStuffAsync(CreateStuffRequest request)
    {
        ...
        if (newStuff == null)
        {
            _logger.LogError("Stuff NotFound");
    ...
    public async Task<bool> DeleteStuffAsync(int id)
    {
        ...
        if (stuff is null)
        {
            _logger.LogError($"Stuff id : {id} NotFound ");
            throw new StuffNotFoundExeption();
        }
    ...
    public async Task<GetStuffResponse?> GetStuffByIdAsync(int id)
    {
        ...
        if (stuff is null)
        {
            _logger.LogError($"Stuff id : {id} is NotFound");
            throw new StuffNotFoundExeption();
        }
        return new GetStuffResponse();
    }
